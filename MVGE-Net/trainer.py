...
def train(model, dataset, epoches, dev_every, loss_function, optimizer, save_path, datasetname,k,log_every=5, max_patience=5):
    debug('Start Training')
    debug(dev_every)#dev_every=len(dataset.train_batches) 224  几个batch
    ...
    best_f1 = 0
    log_flag = 0
    max_steps = epoches * dev_every#100*224
    all_train_acc = []
    all_train_loss = []
    all_valid_acc = []
    all_valid_loss = []
